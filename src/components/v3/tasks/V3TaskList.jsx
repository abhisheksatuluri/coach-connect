...
import { CheckSquare, Calendar, Flag, User, Plus } from 'lucide-react';
...
export default function V3TaskList() {
    ...
    const filtered = tasksData.filter(t => {
        ...
        if (filter === 'Delegated') return false;
        return !t.completed;
    ...
    return (
        <div className="flex flex-col h-full bg-[#FAFAF9]">
            <ScrollShrinkHeader
                title="Tasks"
                showBack={false}
                action={<button className="w-8 h-8 flex items-center justify-center bg-stone-900 text-white rounded-full hover:bg-stone-700 transition-colors"><Plus className="w-4 h-4" /></button>}
            />
                    ...
                    {filtered.map((t, i) => {
                        ...
                        return (
                            <div
                                ...
                                className="bg-white p-4 rounded-2xl border border-stone-200 shadow-sm flex items-start gap-4 hover:shadow-md hover:border-teal-500/50 transition-all cursor-pointer group active:scale-[0.99] duration-200"
                            >
